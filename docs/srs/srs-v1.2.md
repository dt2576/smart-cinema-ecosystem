...
**Source Baseline:** Business Analysis v2.1 + BRD v1.2  
...
| 1.2 | 13/09/2026 | Replaced per-ticket QR model with one Booking QR per paid Booking while retaining individual Ticket states and ticket-level check-in |
...
Trong SRS v1.2, hệ thống chính thức hỗ trợ **Concession Add-on** trong quá trình Booking.
...
| Ticket | Vé điện tử tương ứng với một purchased Seat/Seat Unit; mỗi Ticket có trạng thái Check-in riêng |
| Booking QR | QR duy nhất đại diện cho một paid Booking, dùng để truy xuất Booking và danh sách Ticket liên quan |
| Booking QR Check-in | Staff scan Booking QR, hệ thống load Booking/Tickets và Check-in theo từng Ticket |
| Partial Check-in | Một phần Ticket trong cùng Booking được Check-in trong khi Ticket khác vẫn VALID |
...
| Staff | Nhân viên quầy/soát vé | Scan Booking QR, xem Ticket trong Booking và Check-in Ticket hợp lệ |
...
# 3.10. Ticket & Booking QR — FR-TICKET

| FR ID | Tên | Hành vi | Rules | Main Flow |
|---|---|---|---|---|
| FR-TICKET-001 | Generate Ticket | Tạo Ticket sau Payment Success | Booking PAID | Payment verified → create tickets |
| FR-TICKET-002 | One Ticket per Seat Unit | Một purchased Seat/Seat Unit tạo một Ticket | Không duplicate | Generate per booking seat |
| FR-TICKET-003 | Unique Ticket Identifier | Mỗi Ticket có mã định danh unique | Không duplicate | Generate → persist |
| FR-TICKET-004 | Generate Booking QR | Mỗi paid Booking phải có một Booking QR duy nhất | Một Booking → một Booking QR identity | Booking PAID → generate QR token |
| FR-TICKET-005 | Resolve Booking QR | Backend phải resolve đúng Booking từ Booking QR | Unknown/invalid token reject | Scan token → load booking |
| FR-TICKET-006 | Booking Ticket Retrieval | Booking QR phải cho phép truy xuất danh sách Ticket và trạng thái hiện tại | Chỉ authorized actor | Load booking → tickets |
| FR-TICKET-007 | Customer Booking QR View | Customer xem Booking QR của Booking thuộc quyền sở hữu | Ownership | Authenticate → load QR |
| FR-TICKET-008 | Customer Ticket Status View | Customer xem danh sách Ticket và trạng thái từng Ticket | Ownership | Load booking → ticket states |
| FR-TICKET-009 | No Duplicate Ticket Generation | Retry Payment không sinh thêm Ticket | Idempotent | Check existing → skip/create |
| FR-TICKET-010 | Invalid Ticket Enforcement | CANCELLED/EXPIRED Ticket không được Check-in | Ticket state authoritative | Validate → reject |

---

# 3.11. Booking QR Check-in — FR-CHECKIN

| FR ID | Tên | Hành vi | Rules | Main Flow |
|---|---|---|---|---|
| FR-CHECKIN-001 | Scan Booking QR | Staff scan Booking QR | Authenticated Staff | Scan → submit token |
| FR-CHECKIN-002 | Resolve Booking | Backend resolve Booking từ Booking QR | Unknown token reject | Resolve token → booking |
| FR-CHECKIN-003 | Paid Booking Validation | Booking phải PAID | Otherwise reject | Load booking → validate |
| FR-CHECKIN-004 | Cinema Scope Validation | Booking/Ticket phải thuộc Cinema phù hợp với Staff | Wrong Cinema reject | Resolve cinema → scope check |
| FR-CHECKIN-005 | Showtime Window Validation | Check-in phải tuân time-window policy | Server time | Compare time |
| FR-CHECKIN-006 | Ticket List Retrieval | Hệ thống hiển thị Ticket thuộc Booking cùng trạng thái hiện tại | Booking hợp lệ | Load tickets |
| FR-CHECKIN-007 | Ticket-level Selection | Staff có thể chọn một hoặc nhiều Ticket đủ điều kiện để Check-in | Chỉ VALID tickets selectable | Select eligible tickets |
| FR-CHECKIN-008 | Duplicate Check-in Prevention | Một Ticket không được Check-in thành công lần hai | Atomic ticket transition | Validate state → mark checked-in |
| FR-CHECKIN-009 | Partial Check-in | Booking có thể chứa mixed Ticket states | Ticket khác vẫn VALID nếu chưa Check-in | Update selected only |
| FR-CHECKIN-010 | Re-scan Booking QR | Scan lại Booking QR phải trả trạng thái Ticket mới nhất | QR scan không tự consume Booking | Resolve → refresh states |
| FR-CHECKIN-011 | Successful Check-in Audit | Check-in lưu checkedInAt và Staff trên từng Ticket | Audit required | Persist transition + audit |
| FR-CHECKIN-012 | Concurrent Check-in Protection | Hai concurrent check-in cùng Ticket chỉ có tối đa một success | Atomic/concurrency-safe | Compete → one success |
| FR-CHECKIN-013 | Validation Result | UI nhận result chuẩn hóa cho Booking/Ticket | Clear business result | Return status/result |
...
**DR-020:** Một paid Booking có tối đa một Booking QR identity đang được sử dụng để truy xuất Booking.

**DR-021:** Booking QR Token phải unique và resolve đúng một Booking.

**DR-022:** Booking QR có thể được scan nhiều lần mà không tự động làm mất hiệu lực toàn bộ Booking/Ticket.

**DR-023:** Check-in state được lưu trên từng Ticket, không phải chỉ ở Booking level.

**DR-024:** Các Ticket trong cùng Booking được phép có state khác nhau, ví dụ CHECKED_IN và VALID đồng thời.
...
| Create Payment | Own | ✗ | ✗ | ✗ |
| View Booking/Ticket | Own | Operational | Scope | ✓ |
| Scan Booking QR / Check-in Ticket | ✗ | Scope | Scope if permitted | ✓ |
| Manage Staff | ✗ | ✗ | Scope | ✓ |
| Assign Manager | ✗ | ✗ | ✗ | ✓ |
...
Staff chỉ được sử dụng Booking QR để Check-in các Ticket thuộc Cinema phù hợp.

```text
Staff Cinema A
+
Booking/Ticket Cinema B
=
DENIED
```
...
# 8.4. Booking QR Lifecycle

Booking QR không có lifecycle độc lập phức tạp; nó phản ánh khả năng truy xuất paid Booking.

```text
Booking PAID
   ↓
Booking QR AVAILABLE FOR RETRIEVAL
```

Rules:

- Booking QR dùng để resolve Booking, không phải để đại diện cho một Ticket duy nhất.
- Booking QR có thể scan nhiều lần.
- Việc Check-in làm thay đổi trạng thái từng Ticket, không consume toàn bộ Booking QR.
- Booking CANCELLED/không hợp lệ phải làm các Ticket liên quan không đủ điều kiện Check-in.

---

# 8.5. Ticket Lifecycle
...
| BR-041 | Payment Idempotency | FR-PAYMENT-010, FR-TICKET-009, NFR-CONC-004 |
...
| BR-046 | Booking QR | FR-TICKET-004, 005, 006 |
| BR-047 | Booking & Ticket Ownership | FR-TICKET-007, 008, FR-AUTH-009 |
| BR-048 | Scan Booking QR | FR-CHECKIN-001, 002 |
| BR-049 | Booking & Ticket Validation | FR-CHECKIN-003, 005, 006, 007, 010 |
| BR-050 | Cinema Validation | FR-CHECKIN-004, FR-AUTH-008 |
| BR-051 | Duplicate Check-in Prevention | FR-CHECKIN-008, 012, NFR-CONC-003 |
| BR-052 | Check-in Audit | FR-CHECKIN-011, FR-AUDIT-004 |
...
| BR-062 | Ticket Check-in Audit | FR-CHECKIN-011, FR-AUDIT-004 |
...
Một Ticket chỉ Check-in thành công một lần; Booking QR có thể được scan nhiều lần để truy xuất trạng thái mới nhất.
...
Booking QR uniquely resolves one Booking và có thể được scan nhiều lần.

## INV-12B

Check-in state được quản lý theo từng Ticket; các Ticket trong cùng Booking có thể có trạng thái khác nhau.
...
SRS v1.2 tiếp tục hỗ trợ **Concession/F&B Add-on** vào phạm vi Smart Cinema mà không mở rộng hệ thống thành một nền tảng quản lý F&B hoàn chỉnh.
...
**SRS v1.2 là baseline chính thức thay thế SRS v1.1 và là đầu vào cho các bước thiết kế dữ liệu, API và triển khai tiếp theo.**