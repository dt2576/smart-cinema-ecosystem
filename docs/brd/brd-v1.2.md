...
|---|---|---|
| 1.0 | 09/09/2026 | Initial BRD baseline |
| 1.1 | 09/09/2026 | Added limited Concession/F&B Add-on scope |
| 1.2 | 13/09/2026 | Updated Booking QR model: one Booking QR retrieves multiple individual Tickets; ticket-level check-in retained |
...
| Staff | Internal | Nhân viên vận hành tại rạp | Scan Booking QR, truy xuất Booking/Tickets và thực hiện Check-in theo từng Ticket |
...
|---|---|---|---|
| Customer | External | Người mua vé | Tìm phim, chọn rạp, đặt ghế, chọn Concession, thanh toán, nhận Ticket và sử dụng Booking QR |
...
## 3.1.10A. Concession Add-on

MVP hỗ trợ Concession/F&B dưới dạng add-on trong Booking:

- Customer xem Concession Catalog đang ACTIVE.
- Customer chọn Popcorn, Drink, Combo hoặc item hợp lệ khác.
- Customer thay đổi số lượng hoặc loại bỏ item trước Payment.
- Hệ thống snapshot giá Concession tại thời điểm Booking.
- Concession Amount được Backend cộng vào Final Amount.

MVP không bao gồm Inventory, Warehouse, Supplier, Kitchen, Stock Movement hoặc standalone POS.
...
## 3.1.12. Ticket & Booking QR

Bao gồm:

- Sinh Ticket sau Payment thành công.
- Một Seat/Seat Unit đã mua tương ứng một Ticket.
- Mỗi Ticket có mã định danh riêng và trạng thái riêng.
- Mỗi Booking đã thanh toán có một Booking QR duy nhất để truy xuất toàn bộ Booking.
- Booking QR cho phép truy xuất danh sách Ticket và Concession thuộc Booking.
- Customer có thể xem Booking QR và trạng thái từng Ticket trong Booking.

---

## 3.1.13. Booking QR Check-in

Bao gồm:

- Staff scan Booking QR.
- Backend resolve Booking từ Booking QR.
- Kiểm tra Booking đã PAID.
- Kiểm tra Cinema và Showtime.
- Hiển thị danh sách Ticket thuộc Booking.
- Check-in theo từng Ticket hoặc nhiều Ticket hợp lệ được chọn.
- Cho phép partial check-in trong cùng Booking.
- Chống check-in lặp trên từng Ticket.
- Scan lại Booking QR phải phản ánh trạng thái Ticket mới nhất.
...
| BR-045 | Unique Ticket Identifier | Mỗi Ticket phải có mã định danh duy nhất để quản lý trạng thái và phục vụ Check-in theo từng Ticket. | Must | System |
| BR-046 | Booking QR | Mỗi Booking đã thanh toán phải có một Booking QR duy nhất để truy xuất Booking và toàn bộ Ticket liên quan. | Must | Customer / Staff |
| BR-047 | Booking & Ticket Ownership View | Customer chỉ được xem Booking QR và Ticket thuộc Booking của mình trừ các role có quyền vận hành phù hợp. | Must | Customer / Security |
...
| Req ID | Requirement Name | Description | Priority | Owner / Stakeholder |
|---|---|---|---|---|
| BR-048 | Scan Booking QR | Staff phải có khả năng scan Booking QR để truy xuất Booking và danh sách Ticket liên quan. | Must | Staff |
| BR-049 | Booking & Ticket Validation | Backend phải xác minh Booking tồn tại, đã PAID, đúng Cinema/Showtime và trả trạng thái riêng của từng Ticket trước Check-in. | Must | Staff / System |
...
| BR-052 | Check-in Audit Information | Hệ thống nên lưu Ticket được Check-in, thời gian Check-in và Staff thực hiện. | Should | Manager / Admin |
...
## 4.13A. Concession Add-on Requirements

| Req ID | Requirement Name | Description | Priority | Owner / Stakeholder |
|---|---|---|---|---|
| BR-065 | Concession Add-on Selection | Hệ thống phải cho phép Customer lựa chọn các sản phẩm đồ ăn/nước hoặc combo đang ACTIVE như add-on trong quá trình Booking. | Should | Customer / Business Owner |
| BR-066 | Concession Pricing Snapshot | Giá, tên và số lượng Concession được chọn phải được snapshot tại Booking để thay đổi Catalog sau đó không làm thay đổi giao dịch cũ. | Must | Business Owner |
| BR-067 | Concession Total Integration | Backend phải tính Concession Amount và tích hợp vào Final Booking Amount trước khi tạo Payment. | Must | Customer / Business Owner |
...
Tickets Generated
     ↓
Booking QR Generated
     ↓
Staff Scans Booking QR
     ↓
Booking + Tickets Loaded
     ↓
Selected Ticket(s) Checked-in
```
...
| KPI-11 | Booking QR Validation Time | Thời gian Backend resolve Booking QR và trả danh sách Ticket | **≤ 2 giây mục tiêu** |
...
Một Seat/Seat Unit đã mua sinh một Ticket; một paid Booking có một Booking QR để truy xuất toàn bộ Booking.

## BA-08

Một Ticket chỉ được Check-in thành công một lần trong MVP; Booking QR có thể được scan nhiều lần để lấy trạng thái Ticket mới nhất.
...
Smart Cinema Ecosystem phải cung cấp một nền tảng thống nhất cho một chuỗi rạp có nhiều chi nhánh, cho phép khách hàng tìm kiếm suất chiếu, giữ ghế, chọn Concession, đặt vé, thanh toán, nhận Ticket điện tử và Booking QR; đồng thời cung cấp cho đội ngũ vận hành khả năng quản lý Cinema, Room, Seat, Showtime, Staff, Booking và Check-in theo đúng phạm vi được phân quyền.
...
BRD v1.2 này là tài liệu baseline cho các bước tiếp theo:
