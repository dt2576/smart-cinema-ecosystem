  ...
  return <header className="fixed inset-x-0 top-0 z-40 bg-canvas/85 shadow-md backdrop-blur-xl">
    <div className="mx-auto flex h-20 max-w-7xl items-center justify-between gap-3 px-4 lg:px-10">
      ...
      <div className="flex items-center gap-3">
        <Button variant="secondary" className="hidden sm:inline-flex" onClick={() => onPreview("Sign In")}>Sign In</Button>
    ...
    {menuOpen && <nav id="mobile-navigation" aria-label="Mobile navigation" className="grid border-t border-outline/30 px-4 pb-4 xl:hidden">{navigation}<Button variant="secondary" onClick={() => { setMenuOpen(false); onPreview("Sign In"); }}>Sign In</Button></nav>}
  </header>;
}
